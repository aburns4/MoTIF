...
class UIObject(object):
  ...
  obj_type = attr.ib()
  obj_name = attr.ib()
  word_sequence = attr.ib(eq=False)
  text = attr.ib()
  resource_id = attr.ib()
...
def _build_object_type(android_class, resource_id, ancestor_str=None):
  ...
  for obj_type in UIObjectType:
    if obj_type.name == widget_type.upper() or obj_type.name in widget_type.upper():
      return obj_type

  for obj_type in UIObjectType:
    if obj_type.name == resource_id.upper() or obj_type.name in resource_id.upper():
      return obj_type

  if ancestor_str:
    # try to reduce number of cases where UI object type is unknown
    ancestor_type = ancestor_str.split(' ')[0].split('.')[-1]
    for obj_type in UIObjectType:
      if obj_type.name == ancestor_type.upper() or obj_type.name in ancestor_type.upper():
        return obj_type
...
def _build_etree_from_json(root, json_dict):
  ...
  rx1, ry1, rx2, ry2 = json_dict.get('rel-bounds', [0, 0, 0, 0])
  root.set('rel-bounds', '[%d,%d][%d,%d]' % (rx1, ry1, rx2, ry2))
  ...
  root.set('ancestors', ' '.join(json_dict.get('ancestors', [])))
  root.set(
      'scrollable',
      str(
...
class LeafNode(object):
  ...
  def __init__(self,
    ...
    self.uiobject = UIObject(
        obj_type=_build_object_type(element.get('class'), element.get('resource-id')), # , element.get('ancestors')),
...
class ViewHierarchy(object):
  ...
  def load_json(self, json_content, fp=None):
  ...
  def _get_visible_leaves(self):
    ...
    all_leaves = [element for element in all_elements if self._is_leaf(element)]
    widget_bools = ['widget' in elem.get('class').lower() for elem in all_leaves]
    all_visible_leaves = [
