...
# doing this so step by step instructions 
# are the same across different versions
...
# Android
_CLICK_VERBS = [
    'click', 'tap', 'choose', 'press', 'select', 'launch', 'open', 
    # 'turn on',
    # 'turn off'
]
...
def get_direction(obj):
    obj_loc = obj.grid_location
    for (txt, enum) in _LOCATION_GRID_DICT.items():
        if enum == obj_loc:
            return txt
...
def _get_obj_type_str_motif(ui_object,
                      accept_empty=True):
...
def load_all_single_actions(ui_objs, ui_target_idxs, trace_path):
    action_list = []
    trace_task = " ".join(trace_path.split('/')[-2].split('_'))
    ...
    for t in t_steps:
        ...
        if t[0] is None:
            # swipe action
            # no input content string
            input_content_str = ''
            assert target_object_id == -1
            action_type = common.ActionTypes.SWIPE
        elif t[0].obj_type.value == 4 and t[0].obj_name is not None:
            # EDITTEXT
            # when obj name is None, it is the initial click on text field
            # we keep the click as its own action, handled in else below
            input_content_str = t[0].obj_name
...
def get_nearby_name(chosen, ui_obj_list):
    def _is_ascii(s):
        return all(ord(c) < 128 for c in s)

    try:
        for ui in ui_obj_list:
            if ui.obj_name:
...
def process_resource_id(resource_id):
    info = resource_id.split('/')[-1]
    if '_' in info:
        # split by _ char
        return ' '.join(info.split('_'))
    else:
        # split by camelcase
        splitted = re.sub('([A-Z][a-z]+)', r' \1', re.sub('([A-Z]+)', r' \1', info)).split()
        return ' '.join(splitted)

def get_swipe_info(ui_object, ui_objs, gesture, is_next=True):
     
    input_content_str = ''
    action_type = common.ActionTypes.SWIPE

    verb_str = random.choice(_SWIPE_VERBS)
    swipe_prep_word = random.choice(['to', 'until'])
    ...
    if is_next:
        obj_name = _truncate_name(ui_object.obj_name, config.MAX_OBJ_NAME_WORD_NUM)
        if not obj_name:
            obj_name = get_nearby_name(ui_object, ui_objs)
            obj_direction = get_direction(ui_object)
            was_missing = True
            obj_type = _get_obj_type_str_motif(ui_object, accept_empty=False)
        else:
            obj_type = _get_obj_type_str_motif(ui_object)

        obj_article = random.choice(['', 'the'])
        to_join = []
        if len(obj_article) > 0:
            to_join.append(obj_article)
        if obj_name and len(obj_name) > 0:
            to_join.append(obj_name)
        if len(obj_type) > 0 and (obj_type not in obj_name):
            to_join.append(obj_type)

        obj_desc_str = ' '.join(to_join).encode("ascii", "ignore").decode()
        if obj_direction:
...
def get_type_info(ui_object, ui_objs):
    ...
    direction = None
    input_content_str = ui_object.obj_name.encode("ascii", "ignore").decode().strip()
    action_type = common.ActionTypes.INPUT

    verb_str = random.choice(_TYPE_VERBS)

    obj_name = _truncate_name(ui_object.obj_name, config.MAX_OBJ_NAME_WORD_NUM)
    was_missing = False
    if not obj_name:
        obj_name = get_nearby_name(ui_object, ui_objs)
        direction = get_location(ui_object)
        was_missing = True
        obj_type = _get_obj_type_str_motif(ui_object, accept_empty=False)
    else:
        obj_type = _get_obj_type_str_motif(ui_object)
    ...
    obj_article = random.choice(['', 'the'])
    input_prep_word = random.choice(['into', 'in'])
    to_join = []
    if len(obj_article) > 0:
        to_join.append(obj_article)
    if obj_name and len(obj_name) > 0:
    ...
    obj_desc_str = process_resource_id(ui_object.resource_id).strip(' ')
    if len(obj_desc_str) == 0:
        obj_desc_str = 'text field'
    instr = '%s %s %s %s' % (verb_str, input_content_str, input_prep_word, obj_desc_str)

    return instr, action_type, input_content_str, verb_str, obj_desc_str, was_missing

def get_click_info(ui_object, ui_objs):
     
    direction = None
    input_content_str = ''
    action_type = common.ActionTypes.CLICK

    verb_str = random.choice(_CLICK_VERBS)
    obj_name = _truncate_name(ui_object.obj_name, config.MAX_OBJ_NAME_WORD_NUM)
    was_missing = False
    if not obj_name:
        obj_name = get_nearby_name(ui_object, ui_objs)
        direction = get_direction(ui_object)
        was_missing = True
        obj_type = _get_obj_type_str_motif(ui_object, accept_empty=False)
    else:
        obj_type = _get_obj_type_str_motif(ui_object)
    ...
    obj_desc_str = ' '.join(to_join).encode("ascii", "ignore").decode()
    if direction:
        obj_desc_str = ' '.join([obj_desc_str, direction])
    instr = '%s %s' % (verb_str, obj_desc_str)

    return instr, action_type, input_content_str, verb_str, obj_desc_str, was_missing

def swipe_direction(swipe_gesture):
    first_pt = swipe_gesture[0]
    last_pt = swipe_gesture[1]
    h_dist = last_pt[0] - first_pt[0]
    v_dist = last_pt[1] - first_pt[1]
...
def load_all_actions(ui_objs, ui_target_idxs, trace_path, gestures):
    ...
    for t in t_steps:
        ...
        if ui_object is None:
            ...
            # we keep the above click as its own action
            instr, action_type, input_content_str, verb_str, obj_desc_str, was_missed = get_type_info(ui_object, ui_objs)
            if was_missed:
                missed_tsteps.append('type missing')
        else:
            # no input content string
            # CLICK action
            instr, action_type, input_content_str, verb_str, obj_desc_str, was_missed = get_click_info(ui_object, ui_objs)
            if was_missed:
                missed_tsteps.append('click missing')
    ...
    return action_list, missed_tsteps
...
def _generate_string_seq():
  ...
  Some words will be split by string_utils.tokenizer_with_punctuation
...
def get_real_feature_dict(real_action_list,
                          max_word_num,
                          unused_max_word_length,
                          high_level_goal=False,
                          goal=None,
  ...
  Args:
    real_action_list: List of common.Action() instances
  ...
  """
    ...
    missing_ref = False
    if high_level_goal:
        feature['instruction_str'].append(goal.lower())
        word_id_seq, char_id_seq = string_utils.tokenize_to_ids(goal)
        feature['instruction_word_id_seq'].append(word_id_seq)

    for action in real_action_list:
        ...
        if high_level_goal:
            feature['obj_desc_position_seq'].extend([0, len(word_id_seq)])
            feature['input_str_position_seq'].extend([0, 0]) 
            feature['verb_str_position_seq'].extend([0, len(word_id_seq)])
        else:
            # step by step instruction  
            word_id_seq, char_id_seq = string_utils.tokenize_to_ids(action.instruction_str + '. ')
            ...
            if action.action_type.value == 3:
                start_char = action.instruction_str.find(action.input_content_str)
                if start_char != -1:
                    idxs = string_utils.get_token_pos_from_char_pos(action.instruction_str,
                                                                    start_char,
                                                                    start_char + len(action.input_content_str))
                    feature['input_str_position_seq'].extend(idxs)
                else:
    ...
    phrase_count = len(real_action_list)
    ...
    padded_feature_dict = proto_utils.padding_dictionary(feature, padding_shape,
                                                         padding_type,
                                                         padding_value)
    return padded_feature_dict, missing_ref
