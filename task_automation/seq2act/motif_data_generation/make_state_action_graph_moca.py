...
flags.DEFINE_string(
    "env_dir", "../../moca/data/motif/eval_envs_motif",
...
def update_sag(sag, trace_path, trace_info, raw_id_to_pretty, pretty_idx):
    for i in range(len(trace_info['images'])-1):
        ...
        if curr_uid_pretty in sag:
            already_bbox = [p[1] for p in sag[curr_uid_pretty]['children'][ui_type]]
            already_in = chosen_bbox in already_bbox
            if not already_in:
                sag[curr_uid_pretty]['children'][ui_type].append([action_type, chosen_bbox, next_uid_pretty])
        else:
...
def make_sag(app):
    ...
    all_traces = glob.glob(root + app + '/*')
    root2 = '../data/motif/raw/traces_02_14_21/'
    all_traces += glob.glob(root2 + app + '/*')

    # state action graph that will be filled with
    # pretty uid : {view_id:
    #               children: {ui_type: [[action, bbox, next_resulting_view]]
    #                          }
    #               }
    # key : value structure
    sag = {}
